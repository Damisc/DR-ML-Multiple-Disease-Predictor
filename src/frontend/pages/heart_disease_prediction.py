...
with col1:
    ...
    trestbps = st.number_input("Resting Blood Pressure", 0, 250, 118)
    chol = st.number_input("Insulin", 0, 900, 210)

with col2:
    fbs = st.selectbox("Fasting Blood SUgar > 120 mg/dl", [0, 1])
    restecg = st.number_input("Resting ECG (restecg)]", 0, 2, 1)
    thalach  = st.number_input("Max Heart Rate ", 0, 250, 192)
    exang  = st.selectbox("Exercise Induces Angina", [0, 1])

with col3:
    oldpeak  = st.number_input("Oldpeak (ST depression)", 0.0, 10.0, 0.7)
...
if st.button("🔍 Predict", use_container_width=True):
    ...
    if prediction == 1:
        ...
        st.success("✅ Model Prediciton: Low Risk")
